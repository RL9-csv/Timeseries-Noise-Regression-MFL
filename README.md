...
따라서, 시계열 분해 프레임워크가 아닌, 데이터의 복잡한 패턴과 맥락 그 자체를 학습하는 머신러닝 접근법이 유일한 해법이었습니다.
...
## **Why Not Differencing? A Strategic Decision**
...
2. **정보 보존의 목적:** `groupby('LOT_ID').diff()`를 통한 LOT 그룹별 차분 또한 고려했으나, 차분이 적용되면서 발생하는 시작점에 대한 결측값 발생 때문에, 시작 시점이라는 중요한 정보의 소실을 야기하는 문제가 있습니다. 이런 문제 때문에 대안으로 거시적인 관점에서 신호값의 동적 틀성을 포착 가능한 거시적 관점의 Lag, Rolling, Ewm등의 피처 엔지니어링을 채택하였습니다.
...
3-6. 2차 모델링: 정적 피처 + 내부 신호 피처 + 미시적 rolling 파생 변수 + 미시적 ewm 파생 변수
...
3-12. 하이퍼 파라미터 튜닝 이후 최종 알고리즘 선정

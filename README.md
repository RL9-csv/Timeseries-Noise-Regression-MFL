...
해당 프로젝트는 위 장비에서 수집되는 시계열 센서 데이터의 미래 추세를 예측하여, 생산성과 품질의 딜레마를 해결하는 것을 목표로 합니다.

| Category | Description |
| :--- | :--- |
| **The Problem** | MFL 공정의 딜레마: 잦은 장비 교정은 생산성 저하, 교정 생략은 잠재적 불량 발생이라는 품질 저하 리스크 증가로 이어집니다. 진짜 위험 신호는 급격한 스파이크가 아닌, 장비 상태가 점진적으로 악화되며 나타나는 신호값의 완만한 우상향 추세입니다. |
| **The Solution** | 단순히 다음 값을 맞추는 것을 넘어, 실질적인 대응 시간을 확보할 수 있는 미래(`t+10`) 시점의 신호 추세 및 기울기를 예측하는 머신러닝 모델을 개발했습니다. |
| **Key Result** | 최종 `LightGBM` 모델은 10개 전체 센서 채널에서 시계열 교차검증 기준 **평균 R² 0.728, MAE 0.0065(종속변수 평균: 0.0745)를 달성하며, 선제적 위험 감지의 기술적 가능성을 입증했습니다. |

### **Why This Isn't a Standard Time Series Problem**

이 데이터는 전통적인 시계열 분석의 기본 가정을 근본부터 파괴합니다.
...
따라서, 시계열 분해라는 낡은 프레임워크를 버리고, 데이터의 복잡한 패턴과 맥락 그 자체를 학습하는 머신러닝 접근법이 유일한 해법이었습니다.

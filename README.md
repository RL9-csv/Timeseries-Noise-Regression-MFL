...
2. `learning_rate`의 차이. 학습률 또한 단일 채널에서의 값이 0.14로 글로벌 파라미터의 값인 0.02보다 더 높게 나왔습니다. 학습률이 0.14 또한 그리 높은 값은 아니지만 학습률이 높을 때는 모델이 빠르게 학습을 할 수 있지만 중요한 데이터를 놓칠 수 있다는 위험이 있고, 학습률이 낮을 때는 모델이 신중하게 학습을 하여 좋은 결과를 낼 수 있지만, 시간이 오래 걸립니다. 

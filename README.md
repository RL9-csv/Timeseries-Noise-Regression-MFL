...
| :--- | :--- | :--- |
| `micro_sensor_rolling_min_mean_11` |  | `df[sensor_col].rolling(window=11, min_periods=1).min().mean()` | 
| `micro_sensor_rolling_min_std_11` |  | `df[sensor_col].rolling(window=11, min_periods=1).min().std()` | 
| `micro_sensor_rolling_min_mean_33` |  | `df[sensor_col].rolling(window=33, min_periods=1).min().mean()` |
| `micro_sensor_rolling_min_std_33` |  | `df[sensor_col].rolling(window=33, min_periods=1).min().std()` | 
...
결론: 하이퍼파라미터 튜닝 결과, 모든 평가지표에서 `LGBM`이 `XGBoost`를 앞서며 최종 모델로 선정되었습니다(LGBM: `R²: 0.4894, MAE: 0.0072`, XGBoost: `R²: 0.3954, MAE: 0.0079`). 하지만, 튜닝 이후의 결정 계수 점수(R²: 0.49)가 튜닝 전(R²: 0.66)보다 오히려 낮게 나타나는 결과가 나왔습니다. 코드 검토 결과, 이는 옵투나의 `objective` 함수에 있던 문제로 인해서, 5개 폴드 중 첫번째 폴드 점수만을 기준으로 편향된 최적화가 진행되었기 때문임을 확인했습니다. 물론 파라미터 튜닝 코드에는 문제가 있지만 해당 실험의 목적은 두 모델의 최고의 성능을 찾아서 비교를 하는 것이 아니라, 어떤 모델이 문제에 적합한가를 판단하는 것이였습니다. 따라서 편향된 기준으로 잘못된 코드로 진행된 튜닝이었지만, 동일한 조건에서 `LGBM`이 `XGBoost` 보다 더 나은 성능을 보였기 때문에 재튜닝 과정은 생략하고 `LGBM`을 최종 알고리즘으로 채택하여 다음 실험인 전체 채널에 적용하기로 결정하였습니다.

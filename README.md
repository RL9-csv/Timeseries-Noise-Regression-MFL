...
요약하자면 해당 프로젝트 데이터는 전통적인 시계열 분해의 기본 가정에 해당이 되지 않습니다. 따라서 해당 데이터의 문제를 해결할 방법은 시계열 분해가 아닌 계층적 구조 데어터 자체의 복잡한 패턴과 맥락 그 자체를 피처로 만들어 학습을 할 수 있는 머신러닝 모델의 사용입니다. 
